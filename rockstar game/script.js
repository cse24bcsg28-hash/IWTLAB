document.addEventListener('DOMContentLoaded', () => {
        ...
        const mobileGamesHtml = `
            <a href="#" class="v-card">
                ...
                <div class="v-card-info" style="align-items: center; text-align: center; margin-top: 16px;">
                    ...
                    <button class="btn-solid-primary" onclick="window.location.href='checkout.html?game=Grand+Theft+Auto:+San+Andreas'"
            ...
            <a href="#" class="v-card">
                ...
                <div class="v-card-info" style="align-items: center; text-align: center; margin-top: 16px;">
                    ...
                    <div style="color: #999; font-size: 14px; font-weight: 700; margin-bottom: 12px;">Mobile</div>
                    <button class="btn-solid-primary" onclick="window.location.href='checkout.html?game=Grand+Theft+Auto:+Vice+City'"
                        style="background: transparent; color: white; border: 1px solid #333; padding: 10px 24px; border-radius: 30px; font-weight: 700; cursor: pointer; transition: 0.3s; width: 100%;"><i
                            class="fas fa-download" style="margin-right: 8px;"></i>Buy Now</button>
                </div>
            </a>
            <a href="#" class="v-card">
                ...
                <div class="v-card-info" style="align-items: center; text-align: center; margin-top: 16px;">
                    ...
                    <div style="color: #999; font-size: 14px; font-weight: 700; margin-bottom: 12px;">Mobile</div>
                    <button class="btn-solid-primary" onclick="window.location.href='checkout.html?game=Bully:+Anniversary+Edition'"
                        style="background: transparent; color: white; border: 1px solid #333; padding: 10px 24px; border-radius: 30px; font-weight: 700; cursor: pointer; transition: 0.3s; width: 100%;"><i
                            class="fas fa-download" style="margin-right: 8px;"></i>Buy Now</button>
                </div>
            </a>
            <a href="#" class="v-card">
                ...
                <div class="v-card-info" style="align-items: center; text-align: center; margin-top: 16px;">
                    ...
                    <div style="color: #999; font-size: 14px; font-weight: 700; margin-bottom: 12px;">Mobile</div>
                    <button class="btn-solid-primary" onclick="window.location.href='checkout.html?game=GTA:+Liberty+City+Stories'"
                        style="background: transparent; color: white; border: 1px solid #333; padding: 10px 24px; border-radius: 30px; font-weight: 700; cursor: pointer; transition: 0.3s; width: 100%;"><i
                            class="fas fa-download" style="margin-right: 8px;"></i>Buy Now</button>
                </div>
            </a>
    ...
    // Download Toast Notification Logic
    const downloadBtn = document.querySelector('.dl-btn-windows');
    ...
    if (heroPlayBtn && heroVideo) {
        heroPlayBtn.addEventListener('click', () => {
            if (heroVideo.paused) {
                heroVideo.play();
                heroPlayBtn.innerHTML = '<i class="fas fa-pause" style="font-size: 24px; color: white;"></i>';
            } else {
                heroVideo.pause();
                heroPlayBtn.innerHTML = '<i class="fas fa-play" style="font-size: 24px; color: white; margin-left: 4px;"></i>';
            }
        });
    }
    ...
    // Global authentication check: redirect to login if not authenticated
    const activeUser = sessionStorage.getItem('rockstar_user');
    
    // Allow users to stay on login or signin pages without redirecting them endlessly
    const isAuthPage = window.location.href.includes('login.html') || window.location.href.includes('signin.html');
    
    if (!activeUser && !isAuthPage) {
        window.location.href = 'login.html';
    }
    
    const currentUser = sessionStorage.getItem('rockstar_user');
    ...
    // Expose auth functions to global window object
    window.rockstarAuth = {
        login: function(email) {
            sessionStorage.setItem('rockstar_user', email);
            localStorage.setItem('rockstar_last_user', email);
            // Initialize empty order history if it doesn't exist
            if (!localStorage.getItem('rockstar_orders_' + email)) {
                localStorage.setItem('rockstar_orders_' + email, JSON.stringify([]));
            }
        },
        logout: function() {
            sessionStorage.removeItem('rockstar_user');
            localStorage.removeItem('rockstar_last_user');
            window.location.href = 'signin.html';
        },
        getCurrentUser: function() {
            return sessionStorage.getItem('rockstar_user');
        },
        addOrder: function(itemName, platform, price) {
            const user = this.getCurrentUser();
            if (user) {
                const ordersKey = 'rockstar_orders_' + user;
                const orders = JSON.parse(localStorage.getItem(ordersKey)) || [];
                const exists = orders.some(o => o.item === itemName);
                if (!exists) {
                    orders.push({
                        item: itemName,
                        platform: platform,
